...
async def _run_stream_turn(done_event: dict) -> list[dict]:
    ...
    # reads FERAL_STREAMING at construction and the config loader
    # exports it from ``features.streaming``. The default is now ON
    # (config/loader.py ``DEFAULT_STREAMING``), but a settings file or
    # a shell export can still turn it off, and then
    # ``handle_command_stream`` silently delegates to the non-streaming
    # path and emits no stream_delta frames at all. A test of the
    # streaming path has to own the flag it depends on.
