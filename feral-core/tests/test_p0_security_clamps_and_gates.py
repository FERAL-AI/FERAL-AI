...
def test_runtime_device_skill_is_trusted_only_when_the_device_is_registered():
    """``hwdev_*`` manifests are generated by in-repo code
    (``hardware/capability_skill._safety_for``) from a paired device's own
    self-description, so their ``safe`` tier is first-party. The name alone
    must not be enough: a marketplace package can declare any skill_id, so
    the exemption is proven against the live device registry.
    """
    ...
    fake_state.device_registry.list_devices.return_value = [
        DeviceManifest(device_id="paired_thing", device_type="robot", name="Paired"),
    ]
