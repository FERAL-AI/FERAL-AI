...
<Steps>
  ...
  <Step title="Configure FERAL">
    ...
    export FERAL_FIREBASE_CREDENTIALS="/path/to/firebase-service-account.json"
    ```
    ```json settings.json
    ```
    </CodeGroup>
    ...
    <Warning>
      Earlier versions of this page also showed a `channels.push.fcm`
      block in `settings.json`, and named the variable
      `FERAL_FCM_SERVICE_ACCOUNT_PATH`. Neither exists. `PushChannel`
      takes no config argument at all (`channels/push.py:62-77`) and
      reads **only** environment variables; the Firebase one is
      `FERAL_FIREBASE_CREDENTIALS`.
    </Warning>
