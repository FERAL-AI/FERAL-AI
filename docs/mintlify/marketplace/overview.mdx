...
A publish carries two things, and confusing them is the mistake this
repo made at both ends:

- the **metadata manifest** (`manifest_json`, the form field): `kind`,
  `name`, `version`, description, author, plus the per-kind identifier
  the registry indexes on. This is what the catalog and
...
You write the second one. `feral publish` derives the first from it, in
...
| Kind | `name` comes from | Also sent as |
|---|---|---|
| `skill` | `skill_id` | `skill_id` |
| `daemon` | `node_id` (or `id`) | `node_id` + `capabilities` |
| `app` | `app_id` | `app_id`, `brand`, `entry_surface_id`, `surfaces` |
...
The registry opens the tarball and refuses a skill bundle that FERAL
...
The registry is acceptance-gated. A successful publish lands your item
as `status=submitted` / `visibility=private`, and it is **not
