...
sidebarTitle: "Voice"
description: "Three voice paths optimized for different latency, cost, and quality tradeoffs."
---
...
The default wake phrase ("hey feral") is the only one shipped today. Custom wake-word training is on the post-v1.0 roadmap and **not** currently a CLI command — earlier docs claimed a `feral voice train-wakeword` subcommand that was never implemented. To verify the built-in detector is loaded and microphones are wired, run:
