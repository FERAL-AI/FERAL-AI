...
description: "Bridge Telegram, Slack, Discord, and push notifications into the FERAL session system."
---
...
2. Store the token in the vault. The channels step of the setup wizard
   prompts for it, verifies it against Telegram's `getMe`, and stores it
   under the `FERAL_TELEGRAM_BOT_TOKEN` name the runtime reads:

```bash
feral setup --from-step channels
```

   The web UI's Settings → Channels card writes the same credential.

   <Note>
   `feral key add --provider telegram` is **not** the right command here.
   It stores labeled keys for LLM providers under a namespace the channel
   loader and the Telegram probe never read.
   </Note>
