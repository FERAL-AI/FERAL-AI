...
title: "Memory System"
sidebarTitle: "Memory"
description: "Four-tier memory architecture with hybrid search, wiki compilation, and P2P sync."
---
...
Compilation runs automatically on a schedule. To trigger a manual compile or browse pages, use the HTTP API or the WebUI Memory → Wiki tab:
...
```bash
# Check sync status (HLC lag, peer reachability, WAL backlog)
feral sync status

# Force a sync round-trip with all peers now
feral sync now

# List configured peers + their HLC lag
feral sync peers

# Show this node's HLC node-id
feral sync node-id
```
