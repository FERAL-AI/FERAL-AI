...
- preview: `POST /api/apps/preview` — verifies the bundle and returns what
  the app reaches, the skills it will install, and an `install_token`
- install: `POST /api/apps/install` — spends the token; refused with 403
  without one
