...
> **Status: Beta** — The three autonomy modes and the ApprovalManager are shipped, tested, and enforced on every tool-dispatch path. FERAL's overall maturity stance is Beta / single-operator local use (PyPI `Development Status :: 4 - Beta`), so treat autonomy gating as one layer of defense, not a guarantee — review the escalation rules and deny lists before running in `loose` mode.
