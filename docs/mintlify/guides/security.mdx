...
> **Status: Beta** — BlindVault, permission tiers, and execution sandboxing are shipped and tested, and are the defense-in-depth model FERAL is built around. FERAL's own maturity stance is Beta / single-operator local use (PyPI `Development Status :: 4 - Beta`); this is not a hardened multi-tenant server. Read [Limitations and Caveats](#limitations-and-caveats) before you rely on it.
