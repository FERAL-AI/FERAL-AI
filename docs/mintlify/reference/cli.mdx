...
<Warning>
`feral app install` is **currently broken**. It posts straight to
`POST /api/apps/install`, which now requires an `install_token` from
`POST /api/apps/preview` and answers 403 `preview_required` without one.
The command has not yet been updated to take the two steps and show the
disclosure, so it exits with `Install rejected (403)`. Install from the
Marketplace page in the web client, which does implement the flow. See
