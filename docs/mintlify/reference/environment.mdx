...
when the FERAL API and WebSocket listener are reachable solely through the
reverse proxy you configure as trusted. The backend authenticates the exact
ASGI socket peer address; `X-Forwarded-For` and similar forwarded headers do
not establish trust.
