...
|---|---|---|---|
| `feral_http_requests_total` | counter | `method`, `route`, `status` | `api/server.py` HTTP middleware |
| `feral_http_request_duration_seconds` | histogram | `method`, `route` | `api/server.py` HTTP middleware |
| `feral_llm_429_total` | counter | `provider` | `agents/llm_provider.py` |
| `feral_llm_failover_chain_exhausted_total` | counter | — | `agents/llm_provider.py` |
| `feral_sync_active_peers` | gauge | — | `memory/sync.py` |
| `feral_sync_failures_total` | counter | `reason` | `memory/sync.py` |
| `feral_sync_was_active_recent` | gauge | — | `memory/sync.py` |
| `feral_supervisor_approval_queue` | gauge | — | `agents/supervisor.py` |
| `feral_tool_denials_total` | counter | `tool` | `security/sandbox_policy.py` |
| `feral_sandbox_kills_total` | counter | `reason` | sandbox runner |
| `feral_vault_decrypt_errors_total` | counter | — | `security/vault.py` |
| `feral_ws_active_sessions` | gauge | — | `api/server.py` WS endpoints |
...
3. If the spike correlates with a deploy, roll back via `pip install 'feral-ai==<previous-version>'` and restart.

#### `LLMAllProvidersDown`

1. `feral doctor` to see which providers errored and which keys are configured.
2. Validate API keys via `feral key list` and probe each provider in **Settings → Providers**.
...
1. Drain the queue from the WebUI **Supervisor → Approvals** page.
...
2. Confirm the OS keychain still holds the master key (`feral key status`).
...
1. Define the metric in `feral-core/observability/metrics.py` against
   ...
   docstring naming the eventual emitter module.
2. Add at least one panel in `ops/grafana/feral-overview.json` **or** an
   ...
   otherwise.
3. Add an `emit()` call site inside the owning module. Keep it cheap — the
