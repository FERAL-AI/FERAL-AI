...
The vault is encrypted at rest with **ChaCha20-Poly1305** (AEAD). The encryption key is derived via Argon2id from your master passphrase and held in your OS keychain (macOS Keychain, GNOME Keyring, Windows Credential Manager) so the vault unlocks transparently when you start the brain. FERAL decrypts it in memory and the LLM never sees raw credential values — keys are injected at the HTTP layer at request time.
